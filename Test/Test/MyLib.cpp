...
void Scene::renderScene(int withmap, Shader& ourShader, Shader& skyboxShader, Shader& lampShader, Shader& mappingShader, Camera& camera)
{
	...
	model = glm::mat4(1.0f);
	model = glm::rotate(model, glm::radians( 0.0f), glm::normalize(glm::vec3(1.0, 0.0,1.0))); // rotate the quad to show normal mapping from multiple directions
	model = glm::translate(model, glm::vec3(-1.5f, 1.88f, 3.5f));
	mappingShader.setMat4("model", model);
	mappingShader.setVec3("viewPos", camera.Position);
	mappingShader.setVec3("lightPos", lightPos);
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, mappingDiffuseMap);
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, mappingNormalMap);
	renderWall();
